...
explore_text_selection_comparison <- function(var = NULL, data, select_id,
  ...
  if (!scale %in% db_scales & is.null(val) & !select_id %in% data$ID) {
  ...
  } else if (scale %in% db_scales) {
    db_get(select = gsub(col, var, rcol), from = sprintf("%s_%s", scale, var),
           where = list(ID = select_id))[[1]]
  } else {
...
#' otherwise.
explore_text_check_na <- function(context, data, select_id, vars, time, scale,
  ...
  out <- lapply(c(vl, vr), \(col) {
    ...
    db_scales <- get_from_globalenv("db_scales")
    scale_in_db <- scale %in% db_scales
    val <- if (scale_in_db) {
      db_get(select = gsub("var_left", var, col), from = sprintf("%s_%s", scale, var),
             where = list(ID = select_id))[[1]]
    } else {
      data[[col]][data$ID == select_id]
    }
