...
age_server <- function(id, r) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    names(age_agg_vars) <- sapply(age_agg_vars, \(x) {
      ...
      start <- s_extract("(?<=age_).*(?=_)", x[[1]])

      last <- x[[length(x)]]
      end <- if (last == "age_85") "85plus" else  s_extract("\\d{1,2}$", last)
    ...
    age_graph_fun <- function(var_left, region, year, lang) {
      ...
      ggplot2::ggplot(age_data, ggplot2::aes(x = !!ggplot2::sym("age_group"),
                                             y = !!ggplot2::sym("count"),
                                             alpha = !!ggplot2::sym("selected"))) +
    ...
    var_left_1 <- shiny::reactive({
      ...
      if (identical(vvll, c("85", "85+"))) return("age_agg_85plus")
