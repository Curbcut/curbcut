...
contents <- lapply(all_themes, \(t) {
  ...
  cards <- lapply(all_vars, \(var) {
    ...
    var_title <- cc_t(params$variables$var_title[params$variables$var_code == var], lang = "fr")
    ...
    myImage <- shiny::HTML(sprintf('<img src="data:image/png;base64,%s">', txt))
    ...
    list(box = bslib::value_box(
      title = shiny::p(var_title),
      value = paste0(ordinal_form(percentile_f, lang = "fr"), " percentile") |>  s_sentence(),
      showcase = myImage,
      shiny::p(shiny::HTML(text)),
      full_screen = FALSE,
  ...
  list(nav = bslib::nav(cc_t(t, lang = "fr"), shiny::br(), bslib::card_body_fill(cards_ordered)),
       ranking = ranking,
...
boxes <- mapply(\(ind, ind_name) {
  ...
  if (ind$percentile == "Aucune donnée.") {
    bslib::value_box(
      title = shiny::p(ind$row_title, bsicons::bs_icon(ind$bs_icon)),
...
main_card <- bslib::nav(
  ...
  shiny::br(),
  if (!is.null(main_card_outliers)) shiny::h3("Qu'est-ce qui rend cette région unique ?"),
  if (!is.null(main_card_outliers)) bslib::card_body_fill(main_card_outliers),
  if (!is.null(main_card_normal)) shiny::h3("Qu'est-ce qui rend cette région semblable aux autres ?"),
